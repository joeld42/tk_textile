...
//	assert( _finite(mY2));
  ...
  void vputc(tapnik::Image *img, const float *vertices,float &x,float &y, uint32_t color )
  {
    if ( mIndices )
    {
...
//            printf("V1 %3.2f %3.2f V2 %3.2f %3.2f\n", v1[0], v1[1], v2[0], v2[1]);
            if (v1[1] > maxY) {
                maxY = v1[1];
            }
            ...
            const float sz = (1.0/0.21) * 15;
            img->drawLine( p1[0] *sz, p1[1]*sz, p2[0]*sz, p2[1]*sz, color );
...
public:
  ...
  void initFont(const unsigned char *font)
  {
    ...
    virtual void vprintf( tapnik::Image *img, const char *fmt,...)
    {
        va_list args;
        char wbuff[8192];
        wbuff[8191] = 0;
        va_start(args, fmt );
        vsnprintf(wbuff,8191, fmt, args);
        va_end(args);
        
        const char *scan = wbuff;
        while ( *scan )
        {
            vputc(img, *scan);
            scan++;
  ...
  virtual void vputc( tapnik::Image *img, char c)
  {
    mCharacters[c].vputc( img, mVertices,mXloc,mYloc, mColor );
  }
    
    void pen( uint32_t color )
    {
        mColor = color;
//        printf("Color set to: %u\n", color );
    }
    
    void move( uint32_t x, uint32_t y )
    {
        const float sz = 1.0/ ((1.0/0.21) * 15);
        mXloc = x*sz;
        mYloc = y*sz;
//        printf("text pos %f %f\n", mXloc, mYloc );
    }
